...
#Does this relationship change after normalizing each states' total allocation by population? 
...
q3_dat <- q3_dat %>% left_join() #Join by state abbreviation.
...
q4_lr_plot <- ggplot(q4_dat, aes(x = State, y = Allocation)) + 
  geom_boxplot() + ylim(0, 2500000)
